    ...
    db.query(query, (error, result) => {
      if (error) {
        ...
        if (result.length > 0) {
          ...
          const sql = `insert into  adminuser (companyName, companyEmail, username, password, confirmPassword) VALUES("${params[0]}", "${params[1]}", "${params[2]}", "${params[3]}", "${params[4]}")`;
...
const getUser = async (request, response, next) => {
  try {
    var db = request.db;
    let sql = `SELECT * FROM adminuser WHERE id = ${request.params.id}`;
